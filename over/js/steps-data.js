/* ==========================================================================
   Shuvam Overseas Employment PVT.LTD - Content Data & Renderers
   --------------------------------------------------------------------------
   All step/category/vacancy content lives here as plain data arrays, so it
   can be edited without touching the HTML. Renderers loop over the data to
   build the reusable components:

     - Numbered step list  → used by every step section on procedure.html
     - Sector cards        → the 9 category cards (services.html + home)
     - Employer grid       → the 6 responsibility cards on procedure.html
     - Vacancy cards       → the verified demand grid on index.html
   ...
   Elements opt in with data attributes, e.g.
       ...
       <div class="vacancies-grid" data-cards="vacancies" data-limit="3"></div>

   i18n: every translated string is emitted with a data-i18n key
   (sector.0.title, vac.2.salary, step.recruitment.3.t, ...). js/lang.js
   swaps them to Nepali using the dictionary in js/lang-data.js and restores
   the original English from a cache - so the markup stays the single source
   of truth for English copy.
   ========================================================================== */
...
(function () {
  ...
  /* ------------------------------------------------------------------------
     Data: Verified job demands / flash openings
     Placeholder demand notices - replace with live DoFE-approved listings.
     ref = demand reference shown as a ledger code (never translated).
     ------------------------------------------------------------------------ */
  var VACANCIES = [
    {
      ref: "DEMAND/2026/014",
      ...
      salary: "SAR 2,200 - 2,800 / month",
      skills: ["3+ years trade experience", "ITI / Diploma holder", "Able to read schematics"],
      benefits: "Free accommodation, transport, annual air ticket and medical cover."
    },
    {
      ref: "DEMAND/2026/015",
      status: "Open",
      ...
      salary: "QAR 2,000 - 2,500 / month",
      skills: ["Chiller & VRF experience", "Valid trade certificate", "Basic English"],
      benefits: "Company accommodation, transport allowance and annual ticket."
    },
    {
      ref: "DEMAND/2026/016",
      status: "Closing soon",
      ...
      salary: "KWD 180 - 220 / month",
      skills: ["Age 21 - 40 years", "Physical fitness standard", "Police clearance"],
      benefits: "Free housing, duty meals and medical insurance."
    },
    {
      ref: "DEMAND/2026/017",
      status: "Open",
      ...
      skills: ["Hot kitchen experience", "Hotel / restaurant background", "Menu cost awareness"],
      benefits: "Shared accommodation, transport and service charge."
    },
    {
      ref: "DEMAND/2026/018",
      status: "New",
      ...
      skills: ["6G weld test certified", "Site experience preferred", "Valid passport 2+ years"],
      benefits: "On-site housing, EU-standard contracts and travel covered."
    },
    {
      ref: "DEMAND/2026/019",
      status: "New",
      ...
      salary: "EUR 800 - 1,000 / month",
      skills: ["Hotel housekeeping basics", "Team-oriented", "Basic English"],
      benefits: "Accommodation allowance, meals on duty and annual bonus."
    }
  ];
  ...
  var RECRUITMENT_STEPS = [
    ...
    { title: "Agreement Signing", desc: "A service agreement is signed between Shuvam Overseas Employment PVT.LTD and the employer, defining scope, terms and responsibilities." },
  ...
  function renderSteps(container, data, listKey) {
    var html = "";
    for (var i = 0; i < data.length; i++) {
      var item = data[i];
      var base = "step." + listKey + "." + i;
      html += '<li class="step reveal">' +
                '<span class="step__marker">' + pad(i + 1) + "</span>" +
                '<div class="step__content">' +
                  '<h3 data-i18n="' + base + '.t">' + esc(item.title) + "</h3>" +
                  (item.desc ? '<p data-i18n="' + base + '.d">' + esc(item.desc) + "</p>" : "") +
                "</div>" +
              "</li>";
    }
  ...
  function renderSectors(container, limit) {
    var html = "";
    var count = limit ? Math.min(limit, SECTORS.length) : SECTORS.length;
    for (var i = 0; i < count; i++) {
      ...
      html += '<article class="sector-card reveal">' +
                ...
                '<h3 data-i18n="sector.' + i + '.title">' + esc(s.title) + "</h3>" +
                '<ul class="sector-card__jobs">';
      for (var j = 0; j < s.jobs.length; j++) {
        html += '<li data-i18n="sector.' + i + ".jobs." + j + '">' + esc(s.jobs[j]) + "</li>";
      }
      html += "</ul></article>";
    }
    container.innerHTML = html;
  ...
  function renderEmployer(container) {
    ...
    for (var i = 0; i < EMPLOYER_RESPONSIBILITIES.length; i++) {
      ...
      html += '<article class="employer-card reveal">' +
                ...
                '<h3 data-i18n="employer.' + i + '.title">' + esc(e.title) + "</h3>" +
                '<p data-i18n="employer.' + i + '.desc">' + esc(e.desc) + "</p>" +
              "</article>";
    }
    container.innerHTML = html;
  }

  /* Verified demand / vacancy card */
  function renderVacancies(container, limit) {
    var html = "";
    var count = limit ? Math.min(limit, VACANCIES.length) : VACANCIES.length;
    for (var i = 0; i < count; i++) {
      var v = VACANCIES[i];
      var base = "vac." + i;
      var statusClass = "vacancy__badge--new";
      if (/open/i.test(v.status)) statusClass = "vacancy__badge--open";
      else if (/clos/i.test(v.status)) statusClass = "vacancy__badge--closing";

      html += '<article class="vacancy reveal">' +
                '<div class="vacancy__head">' +
                  '<span class="vacancy__ref">' + esc(v.ref) + "</span>" +
                  '<span class="vacancy__badge ' + statusClass + '" data-i18n="' + base + '.status">' + esc(v.status) + "</span>" +
                "</div>" +
                '<h3 data-i18n="' + base + '.title">' + esc(v.title) + "</h3>" +
                '<p class="vacancy__meta">' +
                  '<span data-i18n="' + base + '.country">' + esc(v.country) + "</span>" +
                  '<span aria-hidden="true"> &middot; </span>' +
                  '<span data-i18n="' + base + '.sector">' + esc(v.sector) + "</span>" +
                "</p>" +
                '<p class="vacancy__salary" data-i18n="' + base + '.salary">' + esc(v.salary) + "</p>" +
                '<p class="vacancy__positions" data-i18n="' + base + '.positions">' + esc(v.positions) + "</p>" +
                '<ul class="vacancy__skills">';
      for (var j = 0; j < v.skills.length; j++) {
        html += '<li data-i18n="' + base + ".skills." + j + '">' + esc(v.skills[j]) + "</li>";
      }
      html += "</ul>" +
                '<p class="vacancy__benefits" data-i18n="' + base + '.benefits">' + esc(v.benefits) + "</p>" +
                '<a class="link" href="contact.html"><span data-i18n="vac.apply">Apply via contact</span>' +
                  '<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>' +
                "</a>" +
              "</article>";
    }
    container.innerHTML = html;
  ...
  function boot() {
    ...
    for (var i = 0; i < stepEls.length; i++) {
      ...
      if (STEP_LISTS[key]) renderSteps(stepEls[i], STEP_LISTS[key], key);
    }

    var sectorEls = document.querySelectorAll("[data-cards='sectors']");
    for (var j = 0; j < sectorEls.length; j++) renderSectors(sectorEls[j], readLimit(sectorEls[j]));
    ...
    var vacancyEls = document.querySelectorAll("[data-cards='vacancies']");
    for (var m = 0; m < vacancyEls.length; m++) renderVacancies(vacancyEls[m], readLimit(vacancyEls[m]));

    /* Tell the rest of the site that late-rendered nodes exist now:
       animations.js observes their .reveal states, lang.js re-applies a
       saved Nepali preference (both listen for this event). */
    document.dispatchEvent(new Event("so:render"));
  }
